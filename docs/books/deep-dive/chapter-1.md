...
- 자바스크립트 같은 매니지드 언어는 메모리의 할당 및 해제를 위한 메모리 관리 기능을 언어 차원에서 담당한다.
  ...
  - 생산성을 확보할 수 있는 장점이 있지만 성능 면에서 손실은 감수할 수밖에 없다.
...
```js title="4가지의 네이밍 컨벤션" showLineNumbers
// 카멜 케이스 (camelCase)
var firstName;
...
```js title="문은 여러 토큰으로 구성된다." showLineNumbers
var score = 100;
...
```js title="선언문, 할당문, 함수 선언문, 조건문, 반복문" showLineNumbers
// 변수 선언문
var x;
...
:::note 원시 타입 <sup>(Primitive Type)</sup>
...
:::note 객체 타입 <sup>(Object Type)</sup>
